...
- **Success 200:** `AuthResponse`
  ...
  ```
  (`avatarUrl`/`coverPhotoUrl` appear only when set; `permissions` is omitted
  when empty.)
- **Errors:**
...
- **Purpose:** Public identity lookup by user id. Used by lms-backend to
  hydrate a profile row the first time it meets a userId and to compose
  `avatarUrl`/`coverPhotoUrl` live on profile reads (auth is the only writer
  of those two fields).
- **Auth required:** no
- **Path params:**
  | Param | Type | Required |
...
- **Purpose:** Public identity lookup by username, for lms-backend to hydrate
...
- **Purpose:** Upload a profile photo for `{id}`. Multipart field `file` with
  `Content-Type: image/*`. The image is re-encoded to lossy WebP (quality 80)
  and stored (local disk or S3); a `FileRecord` is written to the `files`
  collection. **OWNER/MENTOR/MEMBER roles only.**
- **Auth required:** yes (bearer, role OWNER/MENTOR/MEMBER)
- **Multipart:** field `file`, `image/*` content type, ≤ 10 MB body, ≤ 30 M
  decoded pixels.
- **Success 201:**
  ```json
  { "avatarUrl": "http://localhost:8080/api/files/view/64f1a2b3c4d5e6f7a8b9c0d1" }
  ```
- **Errors:** 400 `INVALID_ARGUMENT` for missing `file` field, non-image
  content type, header/decode failure, or pixel-count overflow; 401
  unauthenticated; 403 `ACCESS_DENIED` for non-profile roles; 404
  `RESOURCE_NOT_FOUND` `"User not found: <id>"`.

### POST /api/users/{id}/upload-cover-photo
- **Purpose:** Upload a cover photo for `{id}`. Same mechanics as avatar
  (multipart `file`, re-encoded WebP). **OWNER/MENTOR/MEMBER roles only.**
- **Auth required:** yes (bearer, role OWNER/MENTOR/MEMBER)
- **Multipart:** field `file`, `image/*`.
- **Success 201:**
  ```json
  { "coverPhotoUrl": "http://localhost:8080/api/files/view/64f1a2b3c4d5e6f7a8b9c0d1" }
  ```
- **Errors:** same as avatar upload.

### GET /api/files/{id}
- **Purpose:** Download a stored file's bytes. Non-image files get
...
- **Path params:**
  ...
  |---|---|---|
  | `id` | string (ObjectId) | yes |
- **Success 200:** raw file bytes with the file's content type.
- **Errors:** 404 `RESOURCE_NOT_FOUND` `"File not found: <id>"` (also for
  non-ObjectId ids).

### DELETE /api/files/{id}
- **Purpose:** Delete a stored file (bytes + record). **OWNER/MENTOR/MEMBER
  roles only.**
- **Auth required:** yes (bearer, role OWNER/MENTOR/MEMBER)
- **Path params:**
  | Param | Type | Required |
  ...
  | `id` | string (ObjectId) | yes |
- **Success 204:** no body.
- **Errors:** 401 unauthenticated; 403 `ACCESS_DENIED`; 404 `RESOURCE_NOT_FOUND` `"File not found: <id>"`.

### GET /api/files/view/{id}
- **Purpose:** Inline-view a stored file (no `Content-Disposition` attachment).
  This is the URL written into `avatarUrl`/`coverPhotoUrl`.
- **Auth required:** no
- **Path params:**
  | Param | Type | Required |
  |---|---|---|
  | `id` | string (ObjectId) | yes |
- **Success 200:** raw file bytes with content type.
- **Errors:** 404 `RESOURCE_NOT_FOUND` `"File not found: <id>"`.
...
| `INVALID_ARGUMENT` | 400 | `{"code":"INVALID_ARGUMENT","message":"<reason>",...}` | Bad login credentials, bad/expired verification token, wrong current password, upload problems, unconfigured email verification |
