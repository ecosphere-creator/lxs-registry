...
Batches up to 256 lines or 3s, POSTs NDJSON to `/insert/jsonline` with a
`_stream` field per line (`{service="<name>"}`), retries with backoff, drops
oldest beyond 10k buffered lines.
