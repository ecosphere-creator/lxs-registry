...
- agent: new `LOG_SOURCE=tail:<file>` — tails a regular log file with
  `tail -f -n 0` (never EOFs). Robust for the PM2 integration where services'
  `out_file` points at a shared file; a FIFO reader (cat) would exit on EOF and
  re-read stale content once PM2 opened the path as a regular file.
- 1.0.3..1.0.5: Grafana dashboard provisioning ("Live Logs" home page,
  anonymous), streaming binary downloads, Grafana `--homepath` + top-level dir
  fixes, anonymous access.

## 1.0.2 (2026-08-19)

- server: fix Grafana `--homepath` — must point to the extracted Grafana root
  (the dir containing `conf/`), not the extraction parent. Without it Grafana
  failed with "Could not find config defaults" and the server re-downloaded
  Grafana on every restart. Also cache-detect a prior extraction so restarts
  skip the 430MB download.
